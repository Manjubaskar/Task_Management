...
//			  			   Role role = roleDao.getByRolename(newSignup.getName());
    ...
    /**
     * This method used to cange the role of user by admin
     * @param context
     * @param Authorization
     * @param user
     * @param handler
     */
    public void adminRoleChange(RoutingContext context,String Authorization,User user, Handler<AsyncResult<User>> handler) {
        Future<User> future = Future.future();
        future.setHandler(handler);
        if(Authorization.isEmpty()) {
	        ...
	        try {
	        	 ...
	        	 //This condition used to check the user role admin
	        	  ...
	        	  User name = userDao.getByEmail(user.getEmail());
				  if(name!=null) {
							userDao.adminRoleChange(context,user.getEmail(), user.getRole().getRoleid());
				            future.complete();
				            sendSuccess("Role Updated", context.response(),200);
						  }}
						  else {
							  System.out.print("fail");
							  sendError("Admin only update a Role for user", context.response(),400);
						  }}
					  jedisPool.close();	
			            future.complete();	   	 
				} 
