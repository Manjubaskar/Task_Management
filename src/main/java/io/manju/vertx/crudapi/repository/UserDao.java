...
public class UserDao {
     ...
     */
     ...
     public User getByName(RoutingContext context,String name)
      ...
      /**
       * This method used to change the role for user
       * @param context
       * @param email
       * @param roleid
       */
      public void adminRoleChange(RoutingContext context,String email,Integer roleid)
  		{   
    	  try {
    		  
	            entityManager.getTransaction().begin();
	            Query user = entityManager.createQuery("UPDATE User set role_id='"+roleid+"'  WHERE email='"+email+"'");
	            user.executeUpdate();
        		entityManager.getTransaction().commit();
